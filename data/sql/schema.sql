...
CREATE TABLE lexeme (lexeme_item text, belongs VARCHAR(255), repite BIGINT NOT NULL, percent FLOAT NOT NULL, created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL, PRIMARY KEY(lexeme_item, belongs));
CREATE TABLE spam_stat_view (word text, ndoc BIGINT, nentry BIGINT, PRIMARY KEY(word));
CREATE TABLE table_concat_view (word1 text, ndoc1 BIGINT, nentry1 BIGINT, is_spam SMALLINT, PRIMARY KEY(word1, is_spam));
CREATE INDEX viewer_viewed ON knowledge_base (repite);
CREATE INDEX viewer_viewed_index ON lexeme (repite);
CREATE INDEX word1_index ON table_concat_view (word1);
CREATE INDEX is_spam_index ON table_concat_view (is_spam);
